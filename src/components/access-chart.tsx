...
const VascularAccessChart = () => (
    <div className="bg-white rounded-lg p-2 sm:p-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-4 sm:mb-6 gap-2">
            ...
            <span className="text-xs sm:text-sm text-gray-500">Last updated: 7/25/2025</span>
        </div>
            ...
            {data.map((item, i) => (
                <div key={i} className="flex flex-col sm:flex-row sm:items-center justify-between p-2 sm:p-3 bg-gray-50 rounded-lg gap-2">
                    ...
                    <div className="flex items-center space-x-1 sm:space-x-2">
                        ...
                        <span className={`text-xs sm:text-sm ${item.change.includes('↗') ? 'text-green-600' : 'text-red-600'}`}>
                            {item.change}
                        </span>
                    </div>
                </div>
